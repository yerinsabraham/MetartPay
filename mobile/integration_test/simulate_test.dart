...
// Usage (local):
// METARTPAY_BASE_URL=http://127.0.0.1:5001/metartpay-bac2f/us-central1/api flutter test integration_test/simulate_test.dart
...
void main() {
  ...
  testWidgets('simulate-confirm end-to-end', (WidgetTester tester) async {
    ...
    try {
      final simulateResp = await client.post(simulateUri, headers: {'Content-Type': 'application/json'}, body: json.encode(simulatePayload)).timeout(Duration(seconds: 5));
      ...
      while (attempts < 30 && !success) {
        final statusResp = await client.get(Uri.parse('$baseUrl/transactions/$txId'));
        if (statusResp.statusCode == 200) {
          final statusBody = json.decode(statusResp.body);
          final status = (statusBody['status'] ?? '').toString().toLowerCase();
          if (status == 'confirmed' || status == 'completed' || status == 'settled') {
            success = true;
            break;
          }
        }
      ...
      expect(success, isTrue, reason: 'Transaction did not reach confirmed/completed in time (real backend)');
      client.close();
      ...
      final mockBase = 'http://127.0.0.1:$port';
      // Simple in-memory store
      final Map<String, Map<String, dynamic>> store = {};

      // Server loop
      unawaited(mockServer.forEach((HttpRequest req) async {
        ...
        try {
          ...
          if (req.method == 'GET' && txMatch != null) {
            ...
            if (store.containsKey(txId)) {
              ...
              req.response.headers.contentType = ContentType.json;
              req.response.write(json.encode({'transactionId': txId, 'status': store[txId]!['status']}));
            } else {
      ...
      final simulateResp = await mockClient.post(Uri.parse('$mockBase/simulate-confirm'), headers: {'Content-Type': 'application/json'}, body: json.encode(simulatePayload));
      expect(simulateResp.statusCode, anyOf([200, 201]));
      final simBody = json.decode(simulateResp.body);
      ...
      while (attempts < 20 && !success) {
        final statusResp = await mockClient.get(Uri.parse('$mockBase/transactions/$txId'));
        if (statusResp.statusCode == 200) {
          final statusBody = json.decode(statusResp.body);
          final status = (statusBody['status'] ?? '').toString().toLowerCase();
      ...
      expect(success, isTrue, reason: 'Transaction did not reach confirmed/completed in time (mock backend)');
