      ...
      body: Consumer<MerchantProvider>(
        builder: (context, merchantProvider, _) {
          if (merchantProvider.isLoading &&
              !merchantProvider.hasAttemptedLoad) {
          ...
          if (merchant == null) {
            return _EmptyMerchantView(
              onCreate: () => Navigator.pushNamed(context, '/setup'),
            );
          }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _QuickActionsRow(
                    onCreatePayment: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const CreatePaymentLinkScreen(),
                      ),
                    ),
                    onWallets: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              TextButton(
                                ...
                                child: Text(
                                  _showAllRecent ? 'Show less' : 'Show more',
                                ),
                              ),
                            ],
                          ),
    ...
    return items
        .map(
          (invoice) => ListTile(
            ...
            subtitle: Text(
              '${invoice.chainDisplayName} • ${invoice.statusDisplayName}',
            ),
            trailing: Text(_relativeDate(invoice.createdAt)),
            onTap: () =>
                Navigator.pushNamed(context, '/invoice', arguments: invoice.id),
          ),
        )
        .toList();
  }
...
class _HeaderSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final showNotVerified =
        status == 'not verified' ||
        status == 'not_verified' ||
        status == 'not-verified' ||
        status == 'unverified';
    final brand = Theme.of(context).colorScheme.onPrimary == Colors.white
        ...
        children: [
          ...
          if (showPending || showNotVerified) ...[
            Column(
              ...
              children: [
                Icon(
                  showPending ? Icons.info_outline : Icons.error_outline,
                  color: Colors.orangeAccent.shade200,
                ),
                const SizedBox(height: 4),
                Text(
                  showPending ? 'KYC Pending' : 'KYC Not Verified',
...
class _SnapshotSection extends StatelessWidget {
  ...
  const _SnapshotSection({required this.merchantProvider, super.key});
        ...
        child: Row(
          ...
          children: [
            _FlatActionButton(
              icon: Icons.add,
              label: 'Create\nPayment Link',
              onTap: onCreatePayment,
              tooltip: 'Generate a payment link or QR code',
            ),
            _FlatActionButton(
              icon: Icons.account_balance_wallet,
              label: 'Wallet\nAddresses',
              onTap: onWallets,
              tooltip: 'View crypto addresses for direct deposits',
            ),
            _FlatActionButton(
              icon: Icons.history,
              label: 'History',
...
class _EmptyMerchantView extends StatelessWidget {
  ...
  const _EmptyMerchantView({required this.onCreate, super.key});
