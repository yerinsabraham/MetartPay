...
class _PaymentStatusScreenState extends State<PaymentStatusScreen> {
          ...
          .listen(
            (snap) async {
              ...
              if (oldStatus != null && oldStatus != newTx.status) {
                ...
                _bannerType =
                    newTx.status.toLowerCase() == 'confirmed' ||
                        newTx.status.toLowerCase() == 'completed'
  ...
  void _startPolling() {
    if (_usingLocalSimulation)
      return; // don't poll when using local simulated data
    _pollTimer?.cancel();
    _pollTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
      try {
        ...
        if (kDebugMode &&
            _current == null &&
            err.toString().toLowerCase().contains('transaction not found')) {
          _startLocalSimulation();
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          if (kDebugMode)
            Padding(
              ...
              child: IconButton(
                ...
                onPressed: _usingLocalSimulation
                    ? null
                    : () async {
                        try {
                          final tx = await _paymentService.getTransaction(
                          ...
                          final msg = _usingLocalSimulation
                              ? 'Refresh disabled during local simulation'
                              : 'Refresh failed: $e';
      ...
      body: Stack(
        children: [
          Padding(padding: const EdgeInsets.all(16.0), child: _buildBody()),
          if (_showBanner)
            Positioned(
              top: 0,
              left: 0,
    ...
    return Column(
      ...
      children: [
        ...
        if (tx.confirmations != null)
          Text(
            'confirmations: ${tx.confirmations}/${tx.requiredConfirmations ?? 0}',
          ),
        const Spacer(),
        ElevatedButton.icon(
          icon: const Icon(Icons.check),
          label: const Text('Force re-verify (admin)'),
          onPressed: _reverifyInProgress
              ? null
              : () async {
                  setState(() {
                    _reverifyInProgress = true;
                  });
                  final snack = ScaffoldMessenger.of(context);
                  snack.showSnackBar(
                    const SnackBar(content: Text('Reverify requested...')),
                  );
                  try {
                    final res = await _paymentService.reverifyTransaction(
                      tx.txHash,
                      network: tx.network,
                      tokenAddress: tx.metadata?['tokenAddress'],
                    );
                    ...
                    if (res.containsKey('message')) {
                      ...
                      msg = res['result'].toString();
                        ...
                        res['success'] == true)
                      msg = 'Reverify succeeded';
                    snack.showSnackBar(SnackBar(content: Text(msg)));
                  } catch (e) {
                    snack.hideCurrentSnackBar();
                    snack.showSnackBar(
                      SnackBar(content: Text('Reverify failed: $e')),
                    );
                  } finally {
                    if (mounted)
                      setState(() {
                        _reverifyInProgress = false;
                      });
                  }
                },
        ),
        ...
        if (kDebugMode)
          Padding(
            ...
            child: Text(
              'Debug mode: Firestore listener + polling fallback enabled',
